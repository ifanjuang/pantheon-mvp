...
def test_v2_uses_schema_renderer_without_presentation_adapter():
    ...
    assert 'src="v2_app_native.js"' not in html
...
def test_schema_renderer_exposes_one_contacts_card_per_project_without_participations():
...
def test_schema_renderer_uses_business_card_families_and_fields():
    renderer = (COCKPIT / "v2_app_schema.js").read_text(encoding="utf-8")
    ...
    for family in ("project", "information", "contact", "work", "decision", "tool"):
        assert family in renderer
    assert "item.attributes" in renderer
    assert "normalizeInformation" in renderer
    assert "normalizeWork" in renderer
    assert "normalizeDecision" in renderer
    assert "/v1/agency/projects/${encodeURIComponent(state.project)}/information" in renderer
...
def test_handoff_machine_identity_contract_is_preserved():
    ...
    assert 'wrapper.dataset.baseActedId' in renderer
