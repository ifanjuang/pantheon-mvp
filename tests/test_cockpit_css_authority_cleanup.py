...
def test_retired_visual_stylesheets_are_removed() -> None:
    retired = (
        ...
        "schema_editor.css",
        "contacts_editor.css",
        "information_create.css",
        "project_claim_view.css",
    )
...
def test_three_effects_share_the_complete_recto_coordinate_system() -> None:
    ...
    assert "--effect-1-top, 45%" in cards
    assert "--effect-2-top, 50%" in cards
    assert "--effect-3-top, 55%" in cards
