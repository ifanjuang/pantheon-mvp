...
def test_normalizes_skills_toolsets_and_api_without_governance_promotion():
    ...
    assert [row["tool_id"] for row in rows] == ["hermes-skill-web-research", "hermes-toolset-research", "hermes-api-server"]
...
def test_observer_reads_only_verified_get_surfaces():
    seen = []
    payloads = {
    ...
    assert [entry[0].split("8642", 1)[1] for entry in seen] == ["/v1/skills", "/v1/toolsets", "/v1/capabilities"]
...
def test_missing_key_does_not_invent_runtime_state():
    ...
    assert result == {"status": "not_configured", "observed_at": result["observed_at"], "capabilities": []}
