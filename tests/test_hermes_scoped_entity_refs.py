...
def test_context_pack_refs_use_shared_normalization_and_ordered_deduplication() -> None:
    refs = hermes_scoped_context._entity_refs(
        {
            "included_entities": [
...
def test_invalid_stored_context_ref_keeps_scoped_conflict_message() -> None:
    with pytest.raises(
        ...
        hermes_scoped_context._entity_refs(
...
def test_context_pack_still_requires_at_least_one_admitted_entity() -> None:
    with pytest.raises(
        ...
        hermes_scoped_context._entity_refs({"included_entities": []})
...
def test_admitted_entity_uses_shared_key_but_keeps_exact_scope_gate() -> None:
    ...
    admitted = hermes_scoped_context._admitted_entity(
        context_pack,
        entity_type=" project ",
    ...
    with pytest.raises(
        ...
        hermes_scoped_context._admitted_entity(
            context_pack,
            entity_type="project",
...
def test_scoped_context_keeps_materialization_and_authority_outside_entity_ref() -> None:
    ...
    assert "_materialize_entity" in source
