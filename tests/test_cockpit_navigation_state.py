...
STATE = ROOT / "mvp_vertical" / "cockpit" / "v3" / "collection" / "navigation_state.js"
...
def test_navigation_state_is_free_of_dom_and_swiper() -> None:
    # Comments legitimately name what the module must not depend on, so the
    # purity check looks at code only.
    code = "\n".join(
