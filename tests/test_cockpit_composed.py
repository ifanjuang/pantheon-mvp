...
def test_composed_initializer_replays_owner_and_review_migrations_in_dependency_order(monkeypatch):
    ...
    assert len(connection.statements) == 25
    ...
    assert "CREATE TABLE IF NOT EXISTS doc_document_revision_comments" in connection.statements[10]
    assert "CREATE TABLE IF NOT EXISTS agency_information_projection_metadata" in connection.statements[11]
    assert "CREATE TABLE IF NOT EXISTS work_issue_scope_links" in connection.statements[12]
    assert "CREATE TABLE IF NOT EXISTS agency_decision_requests" in connection.statements[13]
    assert "CREATE TABLE IF NOT EXISTS agency_entity_relations" in connection.statements[14]
    assert "CREATE TABLE IF NOT EXISTS execution_results" in connection.statements[17]
    assert "project_change_variant" in connection.statements[18]
    assert "candidate_execution_id" in connection.statements[19]
    assert "agency_decision_request_scope_refs" in connection.statements[20]
    assert "apu_object" in connection.statements[20]
    assert "CREATE TABLE IF NOT EXISTS knowledge_edit_variants" in connection.statements[21]
    assert "CREATE TABLE IF NOT EXISTS apu_mapping_review_events" in connection.statements[22]
    assert "CREATE TABLE IF NOT EXISTS apu_write_command_candidates" in connection.statements[23]
    assert "expected_owner_revision" in connection.statements[24]
    assert "agency_apu_source_match_command_once" in connection.statements[24]
