...
def test_collector_keeps_independent_sources_and_never_computes_global_health():
    ...
    result = collect_document_runtime_observations(
        paperless_gateway_url="http://paperless-gateway:8082", cockpit_read_key="read-key",
        policy_url="http://pantheon-policy-api:8000", policy_api_key="policy-key",
        docling_url="http://docling-serve:5001", docling_api_key="docling-key",
        hermes_inventory_mode="local_cli", hermes_cli_path="/usr/local/bin/hermes",
        opener=_opener(routes, observed),
        runner=lambda *args, **kwargs: SimpleNamespace(returncode=0, stdout="Installed skills\n- pantheon-document-intake\n", stderr=""),
...
def test_hermes_inventory_uses_fixed_native_list_command_without_shell():
    ...
    def runner(command, **kwargs):
        called["command"] = command; called["kwargs"] = kwargs
        return SimpleNamespace(returncode=0, stdout="pantheon-document-intake\nother-skill\n", stderr="")
    result = observe_hermes_skill_inventory(mode="local_cli", cli_path="hermes", runner=runner, which=lambda name: "/opt/hermes/bin/hermes" if name == "hermes" else None)
