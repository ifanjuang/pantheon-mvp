(() => {
  ...
  function projectAttributeRows(attributes = {}) {
    const labels = {
      budget: "Budget",
      surface_terrain: "Surface terrain",
      surface_existante: "Surface existante",
      surface_projet: "Surface projet",
      emprise: "Emprise",
      parcelles: "Parcelles",
      permit_number: "Permis",
    };
    return Object.entries(attributes || {})
      .filter(([, value]) => value !== null && value !== "" && !(Array.isArray(value) && !value.length))
      .slice(0, 12)
      .map(([key, value]) => [labels[key] || key.replaceAll("_", " "), Array.isArray(value) ? value.join(" · ") : String(value)]);
  }
  ...
  function normalizeProject(item, { selected = false } = {}) {
    ...
    return card({
      ...
      back: [
        ...
        ...projectAttributeRows(item.attributes),
      ],
  ...
  function normalizeDecision(projection) {
    const issue = workData(projection);
    const id = issue.issue_id || crypto.randomUUID();
    return card({
      ...
      summary: issue.decision_question || "Le Travail demande une validation humaine avant de poursuivre ou de produire son résultat définitif.",
      status: "review",
      subject_tags: issue.tags || [],
  ...
  function rebuildGraph() {
    ...
    const decisionIds = state.workIssues
      .filter(item => ["review", "needs_review"].includes(workData(item).status))
      .map(item => putCard(normalizeDecision(item)));
    setChildren("space:decisions", decisionIds);
  ...
  async function loadProject(options = {}) {
    ...
    try {
      ...
      const [information, documents, knowledge, workIssues] = await Promise.all([
      ...
      render();
      setMessage(`Affaire ${matched?.display_name || matched?.code || state.project} chargée.`);
    } catch (error) {
      state.information = [];
      state.legacyDocuments = [];
      state.knowledge = [];
