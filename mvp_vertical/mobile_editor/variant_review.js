(() => {
  ...
  async function requestVariantEdit(kind, requestedVariantCount = 1) {
    ...
    message(requestedVariantCount === 2
      ? "Demande A/B mise en file. La sélection et l’application resteront deux décisions séparées."
  ...
  function renderVariant(requestId, variant, requestStatus) {
    ...
    digest.textContent = variant.replacement_digest?.slice(0, 12) || "digest absent";
  ...
  function renderReview(review) {
    ...
    if (review.variants.length) {
      ...
      waiting.textContent = "Aucune proposition reçue. Une disponibilité Hermes ne vaut pas proposition.";
  ...
  async function loadReviews() {
    if (loadingReviews || !state.current || !state.token || !navigator.onLine) return;
    loadingReviews = true;
    const container = $("edit-reviews");
    $("refresh-reviews").disabled = true;
    try {
