...
METADATA_FIELDS = {
    "declared_project_name", "source_date", "mime_type", "checksum",
...
def _project_exists(conn: psycopg.Connection, project_id: str) -> bool:
    row = conn.execute(
        "SELECT 1 FROM agency_projects WHERE project_id = %s", (project_id,)
    ).fetchone()
    ...
    for index, item in enumerate(candidates):
        ...
        if missing:
            raise SourceIntakeError(
                f"candidate {index + 1} missing: {', '.join(sorted(missing))}"
            )
        unknown = set(item) - required
        if unknown:
            raise SourceIntakeError(
                f"candidate {index + 1} unsupported fields: {', '.join(sorted(unknown))}"
            )
        project_ref = str(item["project_ref"]).strip()
        if not project_ref:
            raise SourceIntakeError("candidate project_ref is required")
        if project_ref in seen:
            raise SourceIntakeError(f"duplicate candidate project_ref: {project_ref}")
        ...
        if not basis:
            raise SourceIntakeError("candidate basis cannot be empty")
        producer = str(item["producer"]).strip()
        created_at = str(item["created_at"]).strip()
        if not producer or not created_at:
            raise SourceIntakeError("candidate producer and created_at are required")
        normalized.append(
            {
...
def _replayed(
    ...
    *,
    ...
    with conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            "SELECT source_id, payload_digest, result_snapshot "
            "FROM agency_source_events WHERE idempotency_key = %s",
            (idempotency_key,),
        )
        row = cur.fetchone()
    if row is None:
    ...
    if row["source_id"] != source_id or row["payload_digest"] != payload_digest:
        raise SourceIdempotencyConflict(
            "idempotency key already belongs to another Source mutation"
        )
...
def _record_event(
    ...
    *,
    ...
    conn.execute(
        ...
        """,
        (
            f"source-event-{uuid.uuid4().hex}", source_id, event_type,
            actor, actor_kind, expected_revision, resulting_revision,
            idempotency_key, _digest(payload), Jsonb(payload), Jsonb(result_snapshot),
        ),
    )
...
def create_source(
    ...
    *,
    ...
    source_kind: str,
    origin_system: str,
    origin_external_ref: str,
    raw_source_ref: str,
    received_at: str | datetime,
    actor: str,
    actor_kind: Literal["human", "hermes", "system"],
    idempotency_key: str,
    origin_producer: str | None = None,
    received_by: str | None = None,
    declared_project_name: str | None = None,
    ...
    if actor_kind == "hermes":
        raise SourceGovernanceGateRequired(
            "Hermes direct Source creation requires an admitted bounded capability"
        )
    ...
    for label, value in {
        ...
        "origin_external_ref": origin_external_ref,
    ...
    payload = {
        ...
        "raw_source_ref": raw_source_ref,
        "received_at": _jsonable(received_at),
        "declared_project_name": declared_project_name,
        "source_date": _jsonable(source_date),
        "mime_type": mime_type,
        "checksum": checksum,
    ...
    replay = _replayed(
        conn, idempotency_key=idempotency_key, source_id=source_id,
        ...
        with conn.transaction():
            conn.execute(
                ...
                INSERT INTO agency_sources (
                    ...
                    mime_type, checksum, confidentiality, metadata,
                    created_by, updated_by
                ) VALUES (
                    %s, %s, %s, %s, %s, %s, %s, %s,
                    'unassigned', %s, %s, %s, %s, %s, %s, %s, %s
                )
                """,
                (
                    source_id, source_kind, origin_system, origin_external_ref,
                    origin_producer, received_by, raw_source_ref, received_at,
                    declared_project_name, source_date, mime_type, checksum,
                    confidentiality, Jsonb(metadata or {}), actor, actor,
                ),
            )
            snapshot = _source_row(conn, source_id)
            _record_event(
                conn, source_id=source_id, event_type="source_created",
                actor=actor, actor_kind=actor_kind, expected_revision=0,
                resulting_revision=1, idempotency_key=idempotency_key,
                payload=payload, result_snapshot=snapshot,
            )
        return snapshot
    except psycopg.errors.UniqueViolation as exc:
        raise SourceIntakeError(
            "source_id or origin identity already exists"
        ) from exc
...
def list_sources(
    ...
    *,
    ...
    with conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            f"SELECT * FROM agency_sources {where} "
            "ORDER BY received_at DESC, source_id LIMIT %s",
            params,
        )
        rows = cur.fetchall()
...
def _mutate(
    ...
    *,
    ...
    if actor_kind == "hermes":
        raise SourceGovernanceGateRequired(
            "Hermes direct Source mutation requires an admitted bounded capability"
        )
    replay = _replayed(
        conn, idempotency_key=idempotency_key, source_id=source_id,
        payload_digest=_digest(payload),
    ...
    with conn.transaction():
        ...
        if current["revision"] != expected_revision:
            raise StaleSourceWrite(
                f"expected Source revision {expected_revision}, current is {current['revision']}"
            )
        ...
        result = conn.execute(
            f"UPDATE agency_sources SET {', '.join(columns)}, "
            "updated_by = %s, revision = revision + 1, updated_at = CURRENT_TIMESTAMP "
        ...
        snapshot = _source_row(conn, source_id)
        _record_event(
            conn, source_id=source_id, event_type=event_type,
...
def update_metadata(
    conn: psycopg.Connection,
    *, source_id: str, changes: dict[str, Any], expected_revision: int,
    actor: str, actor_kind: str, idempotency_key: str,
) -> dict:
    ...
    return _mutate(
        conn, source_id=source_id, expected_revision=expected_revision,
        actor=actor, actor_kind=actor_kind, idempotency_key=idempotency_key,
        event_type="metadata_updated", payload={"changes": changes}, assignments=changes,
    )


def suggest_projects(
    conn: psycopg.Connection,
    *, source_id: str, candidates: list[dict], expected_revision: int,
    actor: str, actor_kind: str, idempotency_key: str,
) -> dict:
    ...
    return _mutate(
        conn, source_id=source_id, expected_revision=expected_revision,
        actor=actor, actor_kind=actor_kind, idempotency_key=idempotency_key,
        event_type="project_links_suggested", payload={"candidates": normalized},
        assignments={
            "candidate_project_refs": normalized,
...
def link_project(
    conn: psycopg.Connection,
    *, source_id: str, project_id: str, expected_revision: int,
    actor: str, actor_kind: str, idempotency_key: str,
) -> dict:
    ...
    return _mutate(
        conn, source_id=source_id, expected_revision=expected_revision,
        actor=actor, actor_kind=actor_kind, idempotency_key=idempotency_key,
        event_type="project_linked", payload={"project_id": project_id},
        assignments={"project_id": project_id, "project_link_status": "linked"},
    )


def unlink_project(
    conn: psycopg.Connection,
    *, source_id: str, expected_revision: int,
    actor: str, actor_kind: str, idempotency_key: str,
) -> dict:
    return _mutate(
        conn, source_id=source_id, expected_revision=expected_revision,
        actor=actor, actor_kind=actor_kind, idempotency_key=idempotency_key,
        event_type="project_unlinked", payload={},
        assignments={"project_id": None, "project_link_status": "unassigned"},
    )
...
def exclude_source(
    ...
    *, source_id: str, expected_revision: int,
    ...
    return _mutate(
        conn, source_id=source_id, expected_revision=expected_revision,
        actor=actor, actor_kind=actor_kind, idempotency_key=idempotency_key,
        event_type="source_excluded", payload={},
        assignments={"project_id": None, "project_link_status": "excluded"},
    )
...
def restore_source(
    ...
    *, source_id: str, expected_revision: int,
    ...
    if current["project_link_status"] != "excluded":
        raise SourceIntakeError("only an excluded Source can be restored")
    return _mutate(
        conn, source_id=source_id, expected_revision=expected_revision,
        actor=actor, actor_kind=actor_kind, idempotency_key=idempotency_key,
        event_type="source_restored", payload={},
        assignments={"project_id": None, "project_link_status": "unassigned"},
    )
...
def relate_contained_source(
    conn: psycopg.Connection,
    *, source_id: str, target_source_id: str, actor: str,
    actor_kind: str, idempotency_key: str,
) -> dict:
    _validate_actor(actor, actor_kind)
    if actor_kind == "hermes":
        raise SourceGovernanceGateRequired(
            "Hermes direct Source relation creation requires an admitted bounded capability"
        )
    ...
    if replay is not None:
        return replay
    relation = {
        "relation_id": f"source-relation-{uuid.uuid4().hex}",
        "source_id": source_id,
    ...
    try:
        with conn.transaction():
            conn.execute(
                """
                INSERT INTO agency_source_relations (
                    relation_id, source_id, target_source_id, relation_type, created_by
                ) VALUES (%s, %s, %s, 'contains', %s)
                """,
                (relation["relation_id"], source_id, target_source_id, actor),
            )
            _record_event(
                conn, source_id=source_id, event_type="source_relation_created",
                actor=actor, actor_kind=actor_kind, expected_revision=0,
                resulting_revision=0, idempotency_key=idempotency_key,
                payload=payload, result_snapshot=relation,
            )
        return relation
    except psycopg.errors.UniqueViolation as exc:
